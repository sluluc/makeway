...
function BookingModal({ c, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(28,27,25,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={onClose}>
      <div style={{ background: "#f8f6f2", width: "100%", maxWidth: 480, position: "relative" }} onClick={e => e.stopPropagation()}>
        
        {/* Header */}
        <div style={{ background: "#1c1b19", padding: "28px 32px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontFamily: "Karla, sans-serif", fontSize: 10, letterSpacing: "2px", textTransform: "uppercase", color: "#8c7355", marginBottom: 8 }}>Request to Book</div>
            <div style={{ fontFamily: "Cormorant, serif", fontSize: 22, color: "#f8f6f2", lineHeight: 1.2 }}>{c.title}</div>
            <div style={{ fontFamily: "Karla, sans-serif", fontSize: 12, color: "#6b6259", marginTop: 4 }}>{c.studio} · {c.date} · ${c.price}</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#6b6259", cursor: "pointer", fontSize: 20, lineHeight: 1, padding: 0, marginLeft: 16 }}>✕</button>
        </div>
        ...
        {status === "success" ? (
          <div style={{ padding: "48px 32px", textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "Cormorant, serif", fontSize: 22, marginBottom: 8 }}>Request sent!</div>
            <div style={{ fontFamily: "Karla, sans-serif", fontSize: 13, color: "#6b6259", lineHeight: 1.7 }}>We'll be in touch within 24 hours to confirm your spot and arrange payment.</div>
            <button onClick={onClose} style={{ marginTop: 24, fontFamily: "Karla, sans-serif", fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", background: "#1c1b19", color: "#f8f6f2", border: "none", padding: "10px 24px", cursor: "pointer" }}>Close</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ padding: "28px 32px" }}>
            <div style={{ marginBottom: 18 }}>
              <label style={{ fontFamily: "Karla, sans-serif", fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", color: "#9e9488", display: "block", marginBottom: 6 }}>Your Name</label>
              <input required value={name} onChange={e => setName(e.target.value)} placeholder="Jane Smith" style={{ width: "100%", padding: "10px 12px", border: "1px solid #e8e4dc", background: "#fff", fontFamily: "Karla, sans-serif", fontSize: 13, color: "#1c1b19", outline: "none" }} />
            </div>
            <div style={{ marginBottom: 18 }}>
              <label style={{ fontFamily: "Karla, sans-serif", fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", color: "#9e9488", display: "block", marginBottom: 6 }}>Email Address</label>
              <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane@email.com" style={{ width: "100%", padding: "10px 12px", border: "1px solid #e8e4dc", background: "#fff", fontFamily: "Karla, sans-serif", fontSize: 13, color: "#1c1b19", outline: "none" }} />
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontFamily: "Karla, sans-serif", fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", color: "#9e9488", display: "block", marginBottom: 6 }}>Any questions? (optional)</label>
              <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="e.g. Do I need any experience?" rows={3} style={{ width: "100%", padding: "10px 12px", border: "1px solid #e8e4dc", background: "#fff", fontFamily: "Karla, sans-serif", fontSize: 13, color: "#1c1b19", outline: "none", resize: "none" }} />
            </div>

            <div style={{ background: "#f0ece5", borderLeft: "2px solid #8c7355", padding: "10px 14px", marginBottom: 20, fontFamily: "Karla, sans-serif", fontSize: 11, color: "#6b6259", lineHeight: 1.6 }}>
              We'll confirm your spot and send a payment link within 24 hours. No payment required now.
            </div>

            <button type="submit" disabled={status === "sending"} style={{ width: "100%", background: "#1c1b19", color: "#f8f6f2", border: "none", padding: "13px", fontFamily: "Karla, sans-serif", fontSize: 11, letterSpacing: "1.5px", textTransform: "uppercase", cursor: "pointer", transition: "background 0.2s" }}>
              {status === "sending" ? "Sending…" : `Request to Book — $${c.price}`}
            </button>
            {status === "error" && <div style={{ fontFamily: "Karla, sans-serif", fontSize: 11, color: "#8c2a2a", marginTop: 10, textAlign: "center" }}>Something went wrong — please try again.</div>}
          </form>
        )}
      </div>
    </div>
...
function Card({ c, saved, setSaved }) {
  ...
  return (
    <>
      {showModal && <BookingModal c={c} onClose={() => setShowModal(false)} />}
      <div className="card">
        ...
        <div className="card-foot">
          ...
          <button className="book-btn" onClick={() => setShowModal(true)}>Book</button>
        </div>
      </div>
    </>
  );
